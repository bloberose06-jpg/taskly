...
export default function WorkspacesPage() {
  ...
  const fetchWorkspaces = async (userId: string) => {
    ...
    // 1. Jobs where user is the client and estado is activo or completado
      ...
      for (const job of clientJobs) {
        ...
        if (app) {
          const { data: profile } = await supabase
            ...
            .single()
          if (profile) otherName = profile.nombre
        }
    ...
    // 2. Applications where user is the freelancer and was accepted
    ...
    <div className="page">
                ...
                <Link href={`/jobs/${ws.jobId}/workspace`} key={ws.jobId} className="ws-card">
                  <div className="ws-left">
                    <div className="ws-top">
                      ...
                      <span className="ws-modalidad" style={{ color: MODALIDAD_COLORS[ws.modalidad] || '#fff' }}>
                        ● {ws.modalidad}
                      </span>
                      <span className={`ws-estado ${ws.estado}`}>{ws.estado}</span>
                    </div>
                    <h2 className="ws-title">{ws.titulo}</h2>
                    <div className="ws-other">
                      <div className="other-avatar">{ws.otherName[0].toUpperCase()}</div>
                      <span className="other-label">{ws.role === 'cliente' ? 'Freelancer:' : 'Cliente:'}</span>
      ...
      <style jsx>{`
        * { box-sizing: border-box; }
        ...
        .ws-card { display: flex; align-items: center; justify-content: space-between; gap: 1.5rem; background: #13131a; border: 1px solid rgba(255,255,255,0.07); border-radius: 16px; padding: 1.5rem; text-decoration: none; transition: border-color 0.2s, transform 0.15s; }
        ...
        .ws-estado.activo      { background: rgba(34,197,94,0.1);  color: #4ade80; border: 1px solid rgba(34,197,94,0.2); }
        ...
        .btn-secondary { background: transparent; border: 1px solid rgba(255,255,255,0.15); color: rgba(255,255,255,0.65); text-decoration: none; border-radius: 10px; padding: 0.75rem 1.5rem; font-size: 0.9rem; font-weight: 600; display: inline-block; }
